...
OUTPUT_FIGURE_DIR = '/net/fs06/d3/rzhuang/TROPOMI_world/code/figure_snapshots_random/'
...
def sample_from_test_set(test_df, n_samples, config):
    """
    Randomly sample from the test set without stratification.
    ...
    print(f"\nRandomly sampling {n_samples} samples from test set ({len(test_df)} total)")
    
    # Simple random sampling
    sampled_df = test_df.sample(n=n_samples, 
                               random_state=config.SAMPLING['random_state'] + 42,
                               replace=False)
    
    print(f"\n--- Test Set Sample Summary ---")
    print(f"Total samples drawn: {len(sampled_df)}")
    
    # Show distribution (for information only, not for balancing)
...
def perform_train_val_test_split_and_sample(df_path, config, output_dir):
    ...
    # Save test samples
    test_samples_path = os.path.join(output_dir, 'test_samples_random.csv')
    ...
    summary_path = os.path.join(output_dir, 'split_summary_random.txt')
    with open(summary_path, 'w') as f:
        f.write("=== Random Train/Val/Test Split Summary ===\n\n")
        ...
        f.write("Test set random sampling:\n")
        ...
        # Distribution in test samples (for information only)
        if 'continent' in test_samples.columns:
            f.write("\nTest Samples - Continent Distribution:\n")
            continent_counts = test_samples['continent'].value_counts()
            for cont, count in continent_counts.items():
                f.write(f"  {cont}: {count}\n")
        
        if 'emission_bin' in test_samples.columns:
            f.write("\nTest Samples - Emission Bin Distribution:\n")
            emission_counts = test_samples['emission_bin'].value_counts().sort_index()
...
try:
    ...
    if not sampled_df.empty:
        ...
        fig.suptitle('Random Test Set Sampling Distribution', fontsize=16)
        ...
        if 'continent' in sampled_df.columns:
            ...
            ax.set_title('Random Sample: Continent Distribution')
        ...
        if 'annual_nox_emission' in sampled_df.columns:
            ...
            ax.set_ylabel('Count')
            ax.set_title('Random Sample: NOx Emissions Distribution')
        ...
        if 'emission_bin' in sampled_df.columns:
            ...
            ax.set_title('Random Sample: Emission Bin Distribution')
            ax.set_xlabel('Emission Bin')
            ax.set_ylabel('Count')
        
        # 4. Comparison: Expected uniform vs actual
        ax = axes[1, 1]
        if 'continent' in sampled_df.columns and 'emission_bin' in sampled_df.columns:
            # Create a simple comparison showing the variation in sampling
            cross_tab = pd.crosstab(sampled_df['continent'], sampled_df['emission_bin'])
            expected_uniform = len(sampled_df) / (len(sampled_df['continent'].unique()) * len(sampled_df['emission_bin'].unique()))
            
            # Plot actual counts as scatter
            ...
            ax.set_title('Random Sample: Count by Continent and Emission Bin')
            ax.grid(True, alpha=0.3)
        
        plt.tight_layout()
        random_sample_vis_path = os.path.join(OUTPUT_FIGURE_DIR, 'random_test_samples_visualization.png')
        plt.savefig(random_sample_vis_path, dpi=300, bbox_inches='tight')
        plt.close()
        print(f"Saved random sampling visualization to: {random_sample_vis_path}")
