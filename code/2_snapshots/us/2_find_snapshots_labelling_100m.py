...
def load_era5_100m_wind(era5_dir, years=range(2019, 2025)):
...
def _utc_to_ns(utc_str_or_np):
    """TROPOMI utc_time (bytes/str 'YYYY-MM-DDTHH:MM:SS.ffffffZ' or numpy) → int64 ns."""
...
def process_file(file_path, power_plants, emissions_df, cities_df, process_id=0, config=None):
    ...
    try:
        ...
        for idx, power_plant in power_plants.iterrows():
            plant_lat = power_plant['Latitude']
            plant_lon = power_plant['Longitude']
            ...
            # Wind driving plume detection: ERA5 100m (bilinear in space, nearest hour
            # in time) at the plant location — replaces the TROPOMI-embedded 10m
            # operational-forecast wind used in the original script. The 100m level
            # is closer to actual plume transport height, so weak-wind cases
            # (where 10m vs 100m direction can differ by >30°) should get more
            # reliable downwind-cone definitions.
            try:
                ...
                logger.warning(f'[Process {process_id}] ERA5 100m wind failed for '
                               f'plant {power_plant.get("Facility_ID","?")}: {e}; '
                               f'falling back to TROPOMI 10m')
                local_wind_u = float(np.ma.filled(wind_u[grid_idx], np.nan))
                local_wind_v = float(np.ma.filled(wind_v[grid_idx], np.nan))
                
            # --- Extract primary data ---
            facility_id = power_plant['Facility_ID']
            year     = datetime.strptime(utc_time_data[0], '%Y-%m-%dT%H:%M:%S.%fZ').year
            country_code = power_plant['State']
            
            # build a proper boolean mask
            mask = (
                (emissions_df['Facility ID'] == facility_id) &
                (emissions_df['Year']        == year)
            )

            # extract the two columns at once
            subset = emissions_df.loc[mask, ['NOx Mass (short tons)', 'Primary Fuel Type']]
            if not subset.empty:
                annual_nox        = subset['NOx Mass (short tons)'].iloc[0]
                primary_fuel_type = subset['Primary Fuel Type'].iloc[0]
            else:
                annual_nox        = np.nan
                primary_fuel_type = 'Unknown'
            
            # --- Run Plume Detection with parameters from config ---
            interfering_sources = prepare_interfering_sources_build_trees_inside(
                target_lat=plant_lat, target_lon=plant_lon, target_plant_id=facility_id,
                all_plants_df_raw=emissions_df.loc[emissions_df['Year'] == year], cities_df_raw=cities_df,
                max_distance_km=config['interf_max_distance_km'],
                plant_in_id_col="Facility ID", plant_in_lat_col="Latitude", plant_in_lon_col="Longitude",
                plant_in_emission_col="NOx Mass (short tons)", city_in_lat_col="latitude", city_in_lon_col="longitude",
            ...
            if results['is_significant']==True:
                logger.info(f"[Process {process_id}] Found significant plume for plant {facility_id} in file {file_name}")
...
def process_file_wrapper(args):
    ...
    if len(args) == 6:
        file_path, power_plants, emissions_df, cities_df, process_id, config = args
        return process_file(file_path, power_plants, emissions_df, cities_df, process_id, config)
    else:
        file_path, power_plants, emissions_df, cities_df, process_id = args
        return process_file(file_path, power_plants, emissions_df, cities_df, process_id)
...
def main():
    ...
    # Load emissions data
    power_plant_path = '/net/fs06/d3/rzhuang/TROPOMI/data/us/facility_emissions_by_plant_comprehensive.csv'
    annual_emissions_path = '/net/fs06/d3/rzhuang/TROPOMI/data/us/annual-emissions-facility-aggregation-2019-2024.csv'
    citys_path = '/net/fs06/d3/rzhuang/TROPOMI/data/world/worldcities.csv'
    logger.info(f"Loading emissions data from {annual_emissions_path} and {power_plant_path}")
    emissions = pd.read_csv(annual_emissions_path)
    power_plants = pd.read_csv(power_plant_path)[:500]
    ...
    era5_dir = '/net/fs06/d3/rzhuang/TROPOMI/data/us/era5/expanded'
    logger.info(f"Loading ERA5 100m wind from {era5_dir}")
    load_era5_100m_wind(era5_dir, years=range(2019, 2025))

    # Find TROPOMI data files
    file_path = '/net/fs06/d3/rzhuang/TROPOMI/data/us/TROPOMI_2019-2024'
    ...
    process_args = [(file, power_plants, emissions, cities, i % num_processes, config) for i, file in enumerate(files)]
